...
class LoanManager:
    ...
    def fetch_data(self):
        ...
        cur = con.cursor()
        cur.execute(
            "select Loan_Id , Name , Year , Rate , Monthly_Payment , Total_Payment , MobileNumber , AadharNumber , Address , Pincode , Amount from customer")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def update(self):
        if self.LoanId.get() == '':
            ...
            cur = con.cursor()
            cur.execute(
                "update customer set Name = ?, MobileNumber = ?, AadharNumber = ?, Address = ?, Pincode = ?, Amount = ?, Year = ?, Rate = ?, Total_Payment = ?, Monthly_Payment = ? where Loan_Id = ?",
                (
                    self.name.get(),
                    self.mob.get(),
                    self.aadhar.get(),
...
cur.execute(
    'create table if not exists customer(Loan_Id varchar(20) primary key,Name varchar(20),MobileNumber varchar(20),AadharNumber varchar(20),Address varchar(20),Pincode varchar(20),Amount varchar(20),Year varchar(20),Rate varchar(20),Monthly_Payment varchar(20),Total_Payment varchar(20))')
